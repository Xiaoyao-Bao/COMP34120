...
public class Agent
{
	...
	private int[] minimax(int height, Board board, boolean maximizingPlayer, int alpha, int beta) {
		...
		if (maximizingPlayer) {
			...
			for(int i = 1; i <= holes; i++) {
				...
				if((System.currentTimeMillis() - startTime > timeToSearch) || (maySwap && System.currentTimeMillis() - startTime > (timeToSearch/2))){
						timeElapsed = true;
						break;
					}
			}
	...
	private int IDDFS()
	{
		...
		timeElapsed = false;
		allGameOver = true;
		long threadEndTime, threadStartTime;
		...
		while(!timeElapsed) {
			for(int depth=0; ; depth++) {
				...
				for(int i=1; i<=holes; i++) {
					...
					if (kalah.isLegalMove(move)) {
						Board b = new Board(kalah.getBoard());
					 	Side nextTurn = Kalah.makeMove(b, move);
						...
						int[] results = minimax(depth, b, ourTurn, alpha, beta);
						...
						if(System.currentTimeMillis() - startTime > timeToSearch){
							System.err.println("Time elapsed, depth reached: " + depth);
							timeElapsed = true;
							break;
						}
					}//if
				...
				///board = lastBoard;
				if(allGameOver) {
					System.err.println("Game over reached, depth reached: " + depth);
					timeElapsed = true;
					break;
				}
				if(timeElapsed)
		...
		while(!timeElapsed) {
			for(int depth = 0; ; depth++) {
				...
				for (int i = 1; i <= holes; i++) {
					...
					if(System.currentTimeMillis() - startTime > (timeToSearch/2)) {
						System.err.println("Time elapsed, depth reached: " + depth);
						timeElapsed = true;
						break;
					}
	...
	public void start()
	{
		try {
			...
			File file = new File("errIDDFS.txt");
